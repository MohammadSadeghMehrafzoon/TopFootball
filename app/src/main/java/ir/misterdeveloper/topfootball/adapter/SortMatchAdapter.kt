...
class SortMatchAdapter(
    private var context: Context,
    private var matchResponse: ArrayList<Match?>? = null,
    private val activity: Activity
) : RecyclerView.Adapter<SortMatchAdapter.SortVH>() {

    private var timeIran: String? = null
    private var dateShamsi: String? = null

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): SortMatchAdapter.SortVH {
        val view: View = LayoutInflater.from(parent.context).inflate(R.layout.card_matchs, null)
        return SortVH(view)
    }

    @RequiresApi(Build.VERSION_CODES.O)
    override fun onBindViewHolder(holder: SortMatchAdapter.SortVH, position: Int) {
    ...
    inner class SortVH(@NonNull itemView: View) : RecyclerView.ViewHolder(itemView) {
